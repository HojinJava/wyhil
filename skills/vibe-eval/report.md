...
사용자가 `/vibe-eval report {milestone-slug}` 를 실행할 때.

## 사전 확인

1. `vibe-sessions/{SLUG}/` 디렉토리가 존재하고 MD 파일이 1개 이상인지 확인
   - 없으면: "❌ `vibe-sessions/{SLUG}/` 에 세션 파일이 없습니다." 출력 후 종료
...
`vibe-sessions/{SLUG}/` 의 모든 `.md` 파일을 읽는다.

각 파일에서 다음 값을 파싱한다:
- **모델명**: `- **모델:**` 뒤의 값
- **마일스톤**: `- **마일스톤:**` 뒤의 값 → MILESTONE_TITLE로 사용
- **작업자**: `- **작업자:**` 뒤의 값
- **날짜**: `- **날짜:**` 뒤의 값
...
- **PR 번호**: `- **PR:** #` 뒤의 숫자 (없으면 "미연결")
- **브랜치**: `- **브랜치:**` 뒤의 값

LEVEL은 MILESTONE_TITLE의 `[L{n}]` 에서 추출.

### 2단계: PR diff 수집

각 세션의 PR 번호가 있는 경우 (숫자인 경우):
```bash
...
PR 번호가 없거나 "미연결"이면: 코드 변경 분석 항목에 `[PR 미연결]` 표시.

### 3단계: 개별 리포트 생성

각 세션 파일에 대해 `reports/{SLUG}/{MODEL_FILENAME}.md` 를 생성한다.
MODEL_FILENAME = 모델명을 소문자로, 공백/점을 하이픈으로 치환.

```markdown
# 세션 분석: {MODEL} × {MILESTONE_TITLE}
...
- **공통 프롬프트 구현율**: 공통 프롬프트의 요구사항 중 몇 %가 구현되었는가. 미구현 항목을 구체적으로 명시.
- **추가 프롬프트 대응**: 추가 프롬프트 {ADDITIONAL_COUNT}개 중 몇 개를 올바르게 처리했는가.
- **코드 품질 소견**: diff를 보고 코드 구조, 패턴, 주목할 점. PR이 없으면 세션 메모 기반으로 분석.
- **총평**: 이 세션의 강점과 약점. L{LEVEL} 레벨 프롬프트에 대한 모델의 대응 방식.
```

### 4단계: 비교 리포트 생성 (세션 2개 이상인 경우)

세션 파일이 2개 이상일 때만 `reports/{SLUG}/comparison.md` 를 생성한다.

```markdown
# 비교 분석: {MILESTONE_TITLE}
...
git commit -m "feat: add vibe-eval report for {SLUG}"
```
...
생성된 파일:
  reports/{SLUG}/{MODEL_1_FILENAME}.md
  reports/{SLUG}/{MODEL_2_FILENAME}.md
  reports/{SLUG}/comparison.md  (모델 2개 이상인 경우)
```
