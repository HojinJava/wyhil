...
진행자가 이슈 번호 또는 링크를 전달해주면, 그것을 AI 모델에게 그대로 넘기면 됩니다.

이슈 본문에 마일스톤 링크와 공통 프롬프트가 포함되어 있으므로, AI 모델이 이슈를 읽으면 무엇을 구현해야 하는지 스스로 파악합니다.

**AI 모델에게 전달하는 방법:**

```
이슈 #42 확인하고 바이브코딩 진행해줘.
https://github.com/HojinJava/wyhil/issues/42
```

또는 이슈 번호만 전달해도 됩니다:

```
GitHub 이슈 #42 기준으로 작업해줘.
```

> AI 모델이 이슈 → 마일스톤 → 공통 프롬프트 순서로 읽고 구현을 시작합니다.
> 완료 후에는 `vibe-sessions/{slug}/{model}.md` 에 세션 기록을 남기고 PR을 생성합니다.
