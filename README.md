...
| 평가지표 | 리뷰 시 기준이 되는 항목 |
| 작업 지시 파일 경로 | 각 모델 브랜치의 `task/{TARGET_DIR}/issue-{N}.md` 위치 안내 |

이슈 생성 시 참여 모델 각 브랜치에 `task/{TARGET_DIR}/issue-{N}.md`가 자동으로 push됩니다.
...
# task/{TARGET_DIR}/issue-{N}.md 파일을 읽고 지시대로 개발해
```
...
PR이 생성되면 리뷰어 모델 브랜치에 `task/{TARGET_DIR}/review-{N}.md`가 자동 push됩니다.
...
- 해당 이슈의 참여 모델을 `task/{TARGET_DIR}/issue-{N}.md` 존재 여부로 파악
