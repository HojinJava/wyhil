...
┌──────────────────────────── 관리자 영역 ─────────────────────────────┐
│                                                                       │
│  1. feature-definitions/{slug}.md 작성                               │
│     └─ 공통 프롬프트 + 평가지표 정의                                 │
│  2. /vibe-eval create feature.md                                     │
│     └─ GitHub Issue 자동 생성                                        │
│        (공통 프롬프트 + 평가지표 + 모델별 브랜치·PR 규칙 포함)       │
│                                                                       │
└──────────────────────────────┬────────────────────────────────────────┘
                               │  Issue #{N} 생성
                               ▼
...
│  [바이브코딩]                                                         │
│  "{이슈 URL} 참고해서 진행해"                                        │
│   → 추가 프롬프트 없음 / 지침서 적용은 허용                          │
│                                                                       │
│  [리뷰]                                                               │
│  "{PR URL} 해당 PR 평가해줘"                                         │
│   → 바이브코딩과 다른 모델에게 평가 요청                             │
│                                                                       │
└──────────┬────────────────────────────────────┬────────────────────────┘
           │ 이슈 링크                           │ PR 링크
           ▼                                     ▼
...
│  [코드 작업]                        [평가 작업]                        │
│  이슈 링크를 읽고                   PR 링크를 읽고                     │
│  → 정해진 순서대로 개발 진행        → 이슈에서 평가지표 확인           │
│  → PR 생성 (브랜치·PR 규칙 준수)   → 정해진 포맷으로 평가서 작성      │
│                                     → PR 댓글 등록                     │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
                               │
              모든 모델 PR 완료 + 리뷰 완료 → 관리자가 Issue 수동 종료
                               │
                      /vibe-eval report
                      비교 리포트 생성
```
...
| `/vibe-eval review {pr-url}` | PR을 읽고 이슈의 평가지표 기준으로 평가서 작성 후 댓글 등록 |
