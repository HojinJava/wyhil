...
git fetch origin {MODEL_BRANCH}
git checkout -b vibe/{MODEL}/{SLUG}-{ISSUE_NUMBER} origin/{MODEL_BRANCH}
...
squash 후 위에서 checkout한 {MODEL_BRANCH} 브랜치를 대상으로 PR을 생성한다.
...
추가 프롬프트: (없으면 없음)
수동 수정: (없으면 없음)
시스템 프롬프트: (CLAUDE.md, 커스텀 인스트럭션, .cursorrules 등 자동 로드된 지시사항. 없으면 없음)
