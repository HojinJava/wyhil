# mall/hello [L1] — 작업 지시 (#57)

## 지금 바로 실행

git fetch origin antigravity
git checkout -b vibe/antigravity/mall-hello-l1-57 origin/antigravity

위 명령 실행 후 mall 폴더 안에서 작업을 진행한다.

## 해야 할 작업

메인함수에서 hello world 출력해줘

## 반드시 지켜야 할 규칙

작업 완료 후 모든 커밋을 1개로 squash한다.
squash 후 위에서 checkout한 antigravity 브랜치를 대상으로 PR을 생성한다.
PR 본문에 `References #57` 를 반드시 포함한다.
PR에 본인 모델 라벨을 추가한다.
...
git mv task/mall/issue-57.md task_done/mall/issue-57.md
git commit -m "chore: move issue-57 task to task_done"
git push origin vibe/antigravity/mall-hello-l1-57
```
