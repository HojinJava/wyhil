...
작업 지시 파일: `issue-{ISSUE_NUMBER}.md` (본인 모델 브랜치에서 확인)
...
[각 모델 — 다른 PC]
git pull → issue-{N}.md 읽기 → 개발 → PR 생성

[GitHub Actions — PR opened 시]
 └→ PR 유효성 검사 (기존)
 └→ review-{N}.md 생성 → 나머지 참여 모델 브랜치에 자동 push

[각 리뷰어 모델 — 다른 PC]
git pull → review-{N}.md 읽기 → PR diff 리뷰 → PR 댓글로 평가서 등록
```
